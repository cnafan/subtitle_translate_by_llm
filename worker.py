...
class TranslationWorker(QThread):
    ...
    def __init__(self, api_base_url, model_id, target_folder, context_size, gpu_layers, verbose_log, concurrency_level, bilingual, overwrite, output_ass=False):
        super().__init__()
    ...
    def process_file(self, file_path):
        ...
        if is_completed and results_map:
            # 3a. 生成 SRT
            self._save_srt(file_path, subs, results_map, total_items)
            
            # 3b. 生成 ASS (如果开启)
            if self.output_ass:
                self._save_ass(file_path, subs, results_map, total_items)
    ...
    def _format_ass_time(self, srt_time):
        """将 pysrt 时间转换为 ASS 格式 (H:MM:SS.CC)"""
        # srt_time: hours, minutes, seconds, milliseconds
        h = srt_time.hours
        m = srt_time.minutes
        s = srt_time.seconds
        # ASS uses centiseconds (hundredths)
        c = srt_time.milliseconds // 10
        return f"{h}:{m:02d}:{s:02d}.{c:02d}"

    def _save_srt(self, file_path, subs, results_map, total_items):
        ...
        for i in range(total_items):
            ...
            new_sub = pysrt.SubRipItem(index=i + 1, start=sub.start, end=sub.end, text=final_text)
        ...
        self.emit_log(f"  -> 已保存 SRT: {os.path.basename(output_path_str)}")

    def _save_ass(self, file_path, subs, results_map, total_items):
        ass_header = [
            "[Script Info]",
            "Title: Generated by Subtitle AI Translator",
            "ScriptType: v4.00+",
            "PlayResX: 1920",
            "PlayResY: 1080",
            ...
            "Style: Default,Microsoft YaHei,54,&H00FFFFFF,&H000000FF,&H00000000,&H00000000,0,0,0,0,100,100,0,0,1,2,2,2,10,10,10,1",
            "",
            "[Events]",
        ...
        for i in range(total_items):
            sub = subs[i]
            translated = results_map.get(i, sub.text).replace('\n', ' ')
            ...
            # 使用样式标签实现中英颜色和大小区分
            # 中文 白色 (Default 样式已定义为白色)
            # 英文 黄色 &H00FFFF&, 字体稍小 \fs40
            if self.bilingual:
                line_text = f"{translated}\\N{{\\fs38\\c&H00FFFF&}}{original}"
            else:
                line_text = f"{translated}"
        ...
        file_str = str(file_path)
        if file_str.lower().endswith('.en.srt'):
            output_path_str = file_str[:-7] + '.ass'
        else:
            output_path_str = str(file_path.with_suffix('.ass'))
            
        with open(output_path_str, 'w', encoding='utf-8-sig') as f:
            f.write(ass_content)
        self.emit_log(f"  -> 已保存 ASS: {os.path.basename(output_path_str)}")

    def run(self):
        try:
            # 1. 创建 logs 目录并初始化会话日志文件
            try:
                log_dir = "logs"
                ...
                session_log_name = f"session_{timestamp}.log"
                self.llm_engine.session_log_path = os.path.join(log_dir, session_log_name)
                self.emit_log(f"已创建调试会话日志: {session_log_name}")
            except Exception as e:
                self.emit_log(f"初始化调试日志目录失败: {e}")
